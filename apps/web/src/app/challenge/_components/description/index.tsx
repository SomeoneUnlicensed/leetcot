    ...
    >
      ...
      <div className="mt-2 flex items-center gap-4">
        ...
        <div className="text-muted-foreground flex items-center gap-2">
          ...
          <span className="text-xs">Обновлено {getRelativeTimeStrict(challenge.updatedAt)}</span>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center gap-3">
        ...
        <Tooltip>
          ...
          <TooltipContent>
            <p>{session.data?.user?.id ? 'В закладки' : 'Войдите, чтобы добавить в закладки'}</p>
          </TooltipContent>
        </Tooltip>
      </div>
      ...
      {challenge.id % 5 === 0 ? (
        <RayFundNote
          variant="banner"
          className="mt-6"
          text="ЛитКот помогает учиться программировать, а фонд «РЭЙ» помогает настоящим кошкам и собакам. Присоединяйтесь!"
        />
      ) : null}
