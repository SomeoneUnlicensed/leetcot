...
But we noticed when we call `landSpacecraft` with the wrong literal type (see [the challenge on literal types](https://leetcot.ru/challenge/literal-types)), there's no error like we might expect:
