import socketserver
from http.server import HTTPServer


class FastHTTPServer(HTTPServer):
    # HTTPServer.server_bind() reverse-resolves the address (socket.getfqdn), which blocks for ~5 s
    # in a container without network before the port starts listening.
    def server_bind(self):
        socketserver.TCPServer.server_bind(self)
        host, port = self.server_address[:2]
        self.server_name = host
        self.server_port = port

import configparser
import sys
from http.server import BaseHTTPRequestHandler

CONFIG = "/etc/inventory/config.ini"
parser = configparser.ConfigParser()
if not parser.read(CONFIG):
    print("2026-03-04 09:15:01 ERROR inventory: не найден файл настроек " + CONFIG)
    print("2026-03-04 09:15:01 ERROR inventory: сервис остановлен")
    sys.exit(1)
PORT = int(parser["inventory"]["listen_port"])


class Handler(BaseHTTPRequestHandler):
    def log_message(self, fmt, *args):
        pass

    def do_GET(self):
        body = "inventory ok\n".encode("utf-8")
        self.send_response(200)
        self.send_header("Content-Type", "text/plain; charset=utf-8")
        self.send_header("Content-Length", str(len(body)))
        self.end_headers()
        self.wfile.write(body)


FastHTTPServer(("127.0.0.1", PORT), Handler).serve_forever()
